...
- Sequence number is incremented by one and acknowledgment number is not incremented during the 3-way handshake process
...
### Out of the following, which network-level session hijacking technique is useful in gaining unauthorized access to a target computer with the help of a trusted host’s IP address?
